...
export default async function start(msg: Message) {
  ...
  if (msg.text() === botConfig.START_KETWORD && msg.talker().self()) {
    const contacts: Contact[] = await room.memberAll();
  ...
  const game: treaterGame = gameInfo.get(room.id) || Object.create(null);
  switch (game.status) {
    case Status.START:
...
function initCard(game: treaterGame) {
  ...
  const players: Contact[] = game.players;
...
function record(msg: Message, game: treaterGame) {
  ...
  const speaker: Contact = msg.talker();
...
function vote(msg: Message, game: treaterGame) {
  const num: number = parseInt(msg.text().replace(/[^0-9]/ig, ''));
